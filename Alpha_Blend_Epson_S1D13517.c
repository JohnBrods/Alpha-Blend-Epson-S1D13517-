...
void main() {
    ...
    Memory_Address1 = Buffer[10];  //10 lion
    Memory_Address2 = Buffer[11];  //11 dark lion
    ...
    Memory_Address0 = Buffer[14];  //OUTPUT BUFFER
    ...
    if(Ready){
     ...
     Memory_buffer = 14;
     Write_Command_EpsonS1D13517(0x2A);
     Write_Data_EpsonS1D13517((Memory_buffer-1)<<4 | 0b0001); //Enable The Display   Single Buffer Display (Default) Page 65
     Write_Command_EpsonS1D13517(0x50);    //REG[50h] Display Control Register PAGE 72
     Write_Data_EpsonS1D13517(0x80);       //bit 7 must be set to update display
     TFT_CS = 1;
     LATA1_bit = 1;
     ...
     alphaValue ++;        //16 hours later it is working.
